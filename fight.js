...
function setupChallengers() {
  ...
  Object.entries(challengers).forEach(([groupName, members]) => {
    ...
    corner.style.backgroundImage = `url(${
      challengersBg[groupName] ?? "/img/background/village.webp"
    })`;
    ...
    Object.values(members).forEach((pnj) => {
      ...
      icon.innerHTML = `
                <span class="icon-container" style="background-image: url('./img/challengers/${groupName.toLowerCase()}/${
        pnj.pic
      }'); background-size: cover;" data-name="${pnj.name}">
...
function setupArena(pnj, message) {
  ...
  const arena = `
            <div class="arena" style= "background-image:url(${
              challengersBg[currentGroup] ?? "/img/background/village.webp"
            })"> 
                <div>
                    ...
                    <span class="health-bar">
                            <span class="health"
                            style="width: calc((${currentMonster.currentHp} / ${
                ...
                <div>
                    ...
                    <div class="monster-ui">
                        <div>
                            ...
                            <p>Lv.${
                              monster.level === 10 ? " MAX" : monster.level
                            }</p>
                        </div>
                        <span class="health-bar">
                            <span class="health"
                            style="width: calc((${monster.currentHp} / ${
    monster.maxHp
  }) * 100%);
                            background-color: ${getHealthColor(
                              monster.currentHp,
